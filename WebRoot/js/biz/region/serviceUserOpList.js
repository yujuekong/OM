...
function initTable() {
    if (oTable == null) {
        oTable = $("#userOp_list").dataTable({
            ...
            "sAjaxSource": ROOT_PATH + "/view/sys/sysUserOp/querySysUserOpPage.action",  //异步请求地址
            "aoColumns": [{"mDataProp": "userOpId"},
                          {"mDataProp": "userName"},
                          {"mDataProp": "userId"},
                          {"mDataProp": "bizType"},
                          {"mDataProp": "opDesc"},
            ...
            "aoColumnDefs": [{"bSortable": false, "aTargets": [0, 1, 2, 3, 4, 5, 6,7]},  //这句话意思是第1,3,6,7,8,9列（从0开始算） 不能排序
                             {//和aoColums类似，但他可以给指定列附近爱属性
			                "searchable": false,
			                "orderable": false,
			                "targets": 0,
			                "aTargets": [1], "mRender": function (data, type, full) {
			                    return "<a   onclick='queryUserOpInfo(\"" + full.userId + "\")'>" + data + "</a>"
			                }
            }],
...
//查询详情
function queryUserOpInfo(id) {
	window.location.href = ROOT_PATH + "/view/sys/sysUserOp/querySysUserOpById.action?userId=" + id;
}
