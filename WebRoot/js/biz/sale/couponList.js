    ...
    if (oTable == null) {
        oTable = $("#couponList").dataTable({
            ...
            "aoColumns": [
                ...
                {"mDataProp": "startDate"},
                {"mDataProp": "endDate"},
                {"mDataProp": "couponAmount"},
                {"mDataProp": "couponNumber"},
                {
                    "mDataProp": "couponType", "sClass": "center", "mRender": function (data, type, full) {
                    var couponType = '';
                    if (data == '0') {
                        couponType = '<label class="label label-lg label-success">现金</label>';
                    }
                    else if (data == '1') {
                        couponType = '<label class="label label-lg label-info">折扣</label>';
                    }
                    return couponType;
                }
                },
                {
                    ...
                    actionDiv = '<div class="hidden-sm hidden-xs btn-group">' +
                        ...
                        '<button onclick="preAddOrModifyCoupon(\'' + full.couponId + '\')" class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button>' +
            ...
            "aoColumnDefs": [//和aoColums类似，但他可以给指定列附近爱属性
                ...
                {
                    "sDefaultContent": '', "aTargets": [0, 1, 2, 3, 4, 5, 6]
                }
            ],
...
function preAddOrModifyCoupon(id) {
    var url = ROOT_PATH + "/view/sale/coupon/preAddOrModifyCoupon.action?couponId=" + id;
