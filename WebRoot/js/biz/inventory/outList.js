...
function filterRoleName() {
    ...
    $.ajax({
        url: ROOT_PATH + "/view/inventory/deliveryOrder/getUserRoleName.action",
        data: params,
        type: "post",
        dataType: "json",
        async: false,
        success: function (data) {
            //alert(data);
            if (data != null) {
                var roleName = $("#roleName").html(JSON.stringify(data));
                //alert(includeActivity(data));
                if (includeActivity(data) == true) {
                    //$("#roleFilter").show();
                    $("#roleFilter").attr("hidden", false);
                }
            }
        }
    });
}
function includeActivity(data) {
...
function initTable() {
    ...
    var isLeader = includeLeader(json);
    //alert(isLeader);
    if (oTable == null) {
        oTable = $("#out_list").dataTable({
            ...
            "aoColumns": [
                ...
                //{"mDataProp": "cleanUser.realName"},
                {
                    "mDataProp": "deliveryStatus", "mRender": function (data, type, full) {
                    var deliveryStatus = '';
                    if (data == '1') {
                        deliveryStatus = '<button class="btn btn-minier btn-success" type="button" disabled>已出库</button>';
                    }
                    else if (data == '0') {
                        deliveryStatus = '<button onclick="modifyDeliveryStatus(\'' + full.deliveryOrderId + '\')" class="btn btn-minier btn-info" type="button" disabled >未出库</button>';

                    }
                    else if (data == '2' && isLeader==true) {
                        deliveryStatus = '<button onclick="modifyDeliveryStatus(\'' + full.deliveryOrderId + '\')" class="btn btn-minier btn-info" type="button" >待审核</button>';
                ...
                },
                {
                    ...
                    actionDiv = '<div class="hidden-sm hidden-xs btn-group">' +
                        '<button onclick="orderInDet(\'' + full.deliveryOrderId + '\')" class="btn btn-xs btn-success" style="text-align: center"><i style="text-align: center" class="ace-icon fa fa-search-plus bigger-120">明细</i></button>';
                    actionDiv += '</div>';
                    return actionDiv;
                }
                },
                {
                    ...
                    var actionDiv = '';
                    actionDiv = '<div class="hidden-sm hidden-xs btn-group">' +
                        '<button onclick="queryDeliveryOrderById(\'' + full.deliveryOrderId + '\')" class="btn btn-xs btn-success"><i class="ace-icon fa fa-search-plus bigger-120"></i></button>';
                    //'<button onclick="preAddOrModifyDeliveryOrder(\'' + full.deliveryOrderId + '\')" class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button>' +
                    //'<button onclick="deleteDeliveryOrder(\'' + full.deliveryOrderId + '\')" class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash-o bigger-120"></i></button>';
                    actionDiv += '</div>';
                    return actionDiv;
                }
                }
...
//修改处理状态
function modifyIsAgentStatus(id) {
    var param = {
        "deliveryOrderId": id,
    };
    if (window.confirm("确定修改为已处理")) {
        $.ajax({
            type: "post",
